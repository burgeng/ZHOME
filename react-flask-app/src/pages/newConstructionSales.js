...
import NewConstructionSalesChart from './newConstructionSaleschart'; // Import the chart component

function NewConstructionSales({ localityType }) {
  ...
  useEffect(() => {
    if (localityType) {
      setError(null);
      const route = `/get_localities_newConstructionSales?type=${localityType}&page=${currentPage}&limit=${optionsPerPage}`;
      fetch(route)
        .then((response) => {
          if (!response.ok) {
  ...
  const fetchDataForLocality = (locality) => {
    ...
    const route = `/get_newConstructionSales?type=${localityType}&name=${encodeURIComponent(locality.regionname)}&state=${encodeURIComponent(locality.state)}`;
    fetch(route)
      .then((response) => {
        if (!response.ok) {
  ...
  return (
    <div>
      <h2>Total Sales of Newly Constructed Homes</h2>
      ...
      {selectedLocality && data && (
        <div style={{ marginTop: '20px' }}>
          <h3>Total Count of Finalized Sales for Newly Constructed Homes in {selectedLocality.regionname}, {selectedLocality.state}:</h3>
          <NewConstructionSalesChart data={data} />
...
export default NewConstructionSales;
